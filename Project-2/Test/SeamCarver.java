...
private double[][] cummulativeMatrix(double[][] energy) {
     ...
     private int[] Seam(double[][] C_matrix) {

        int noOfRows = C_matrix.length;
        
        int noOfColumns = C_matrix[0].length;
        ...
        for (int i = noOfRows - 1; i > 0; i--) {
            ...
            if (i < noOfRows - 1 && i > 0) {
                ...
                if (n - 1 >= 0 &&  n - 1 < noOfColumns - 1 &&  n + 1 <= noOfColumns - 1) {
                    ...
                    Min = Math.min(C_matrix[i][n - 1], Math.min(C_matrix[i][n], C_matrix[i][n + 1]));
                ...
                if (n - 1 < 0) {
                    Min = Math.min(C_matrix[i][n], C_matrix[i][n + 1]);
                    for (int j = 0; j < noOfColumns; j++) {
                        if (Min == C_matrix[i][j]) {
                            SeamArray[i] = j;
                            break;
                ...
                if (n + 1 > noOfColumns - 1) {
                    Min = Math.min(C_matrix[i][n - 1], C_matrix[i][n]);
                    for (int j = 0; j < noOfColumns; j++) {
                        if (Min == C_matrix[i][j]) {
                            SeamArray[i] = j;
                            break;
    ...
    public int[] findVerticalSeam() {
        double[][] C_mat = cummulativeMatrix(energyMatrix(picture.height(), picture.width()));
        return Seam(C_mat);
